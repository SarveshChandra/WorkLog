...
enum CalendarInterviewImporter {
        ...
        for event in events.sorted(by: eventSort) {
            if let roundLocation = findRound(for: event, in: opportunities) {
                ...
                if refreshedRound == existingRound {
                    ...
                    ensureCalendarAutoFollowUp(
                        on: &opportunities[roundLocation.opportunityIndex],
                        for: refreshedRound,
                        now: now
                    )
            ...
            ensureCalendarAutoFollowUp(
                on: &opportunities[opportunityIndex],
                for: importedRound,
                now: now
            )
    ...
    private static func ensureCalendarAutoFollowUp(
        on opportunity: inout InterviewOpportunity,
        for round: InterviewRound,
        now: Date
    ) {
        guard round.statusOption(now: now) == .upcoming else { return }

        if isUnlinkedCalendarAutoFollowUp(opportunity, for: round) {
            applyCalendarAutoFollowUp(on: &opportunity, for: round)
            return
        }

        if opportunity.nextActionWasAutoGenerated,
           opportunity.nextActionLinkedRoundID == round.id,
    ...
    private static func isUnlinkedCalendarAutoFollowUp(
        _ opportunity: InterviewOpportunity,
        for round: InterviewRound
    ) -> Bool {
        guard !opportunity.nextActionWasAutoGenerated,
              opportunity.nextActionLinkedRoundID == nil,
              let dueDate = opportunity.nextActionDueDate else {
            return false
        }

        let roundLabel = round.roundName.isBlank ? "interview" : round.roundName
        return opportunity.nextAction == "Prepare for \(roundLabel)"
            && abs(dueDate.timeIntervalSince(round.date)) < 1
    }

    private static func applyCalendarAutoFollowUp(
        on opportunity: inout InterviewOpportunity,
        for round: InterviewRound
    ) {
        let roundLabel = round.roundName.isBlank ? "interview" : round.roundName
        opportunity.nextAction = "Prepare for \(roundLabel)"
        opportunity.nextActionDueDate = round.date
        opportunity.nextActionLinkedRoundID = round.id
