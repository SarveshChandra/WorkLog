...
final class AppStore: ObservableObject {
    ...
    nonisolated static func updatedInterviewOpportunity(
        ...
        reconcileFollowUps(on: &updated, existing: existing)
        ensureUpcomingRoundFollowUps(on: &updated, now: now)
    ...
    nonisolated private static func reconcileFollowUps(
        on opportunity: inout InterviewOpportunity,
        existing: InterviewOpportunity
    ) {
        let existingFollowUpsByID = Dictionary(uniqueKeysWithValues: existing.followUps.map { ($0.id, $0) })

        for index in opportunity.followUps.indices {
            if let existingFollowUp = existingFollowUpsByID[opportunity.followUps[index].id],
               existingFollowUp.wasAutoGenerated,
               followUpWasManuallyChanged(existing: existingFollowUp, updated: opportunity.followUps[index]) {
                opportunity.followUps[index].wasAutoGenerated = false
            }

            if let linkedRoundID = opportunity.followUps[index].linkedRoundID,
               !opportunity.interviewRounds.contains(where: { $0.id == linkedRoundID }) {
                opportunity.followUps[index].linkedRoundID = nil
                opportunity.followUps[index].wasAutoGenerated = false
            }
        }
    }
    ...
    nonisolated private static func followUpWasManuallyChanged(
        existing: InterviewFollowUp,
        updated: InterviewFollowUp
    ) -> Bool {
    ...
    nonisolated private static func ensureUpcomingRoundFollowUps(on opportunity: inout InterviewOpportunity, now: Date) {
        opportunity.ensureUpcomingRoundFollowUps(relativeTo: now)
    }
    ...
    private func migrateDemoInterviewFieldsIfNeeded() {
        ...
        for index in data.interviewOpportunities.indices {
            ...
            case "DesignOps Studio":
                ...
                for followUpIndex in data.interviewOpportunities[index].followUps.indices {
                    if data.interviewOpportunities[index].followUps[followUpIndex].action == "Find referral contact" {
                        data.interviewOpportunities[index].followUps[followUpIndex].action = "Find referral PoC"
                        changed = true
                    }
                }
            default:
