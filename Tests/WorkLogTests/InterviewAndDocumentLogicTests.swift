...
final class InterviewAndDocumentLogicTests: XCTestCase {
    ...
    func testPastDueManualFollowUpRemainsOpenAndBecomesDue() {
        let now = Date(timeIntervalSince1970: 1_790_000_000)
        ...
        XCTAssertEqual(opportunity.activeOrDueFollowUps(relativeTo: now).first?.status(relativeTo: now), .due)
    }
    ...
    func testManualFollowUpDefaultDueTimeStaysActiveAfterToggle() {
        ...
        XCTAssertTrue(opportunity.hasCurrentFollowUp(relativeTo: now))
        XCTAssertEqual(opportunity.activeOrDueFollowUps(relativeTo: now).first?.status(relativeTo: now), .active)
    }
    ...
    func testSavingUpcomingRoundAddsFollowUpWhenMissing() throws {
        ...
        let autoFollowUp = try XCTUnwrap(updated.followUps.first)
        XCTAssertEqual(updated.followUps.count, 1)
        XCTAssertEqual(autoFollowUp.action, "Prepare for Technical")
        XCTAssertEqual(autoFollowUp.dueDate, futureRoundDate)
        XCTAssertEqual(autoFollowUp.linkedRoundID, updated.interviewRounds.first?.id)
        XCTAssertTrue(autoFollowUp.wasAutoGenerated)
        XCTAssertTrue(updated.hasCurrentFollowUp(relativeTo: now))
    }

    func testSavingUpcomingRoundAddsAutoFollowUpWithoutRemovingPastDueManualFollowUp() throws {
        let now = Date(timeIntervalSince1970: 1_790_000_000)
        let futureRoundDate = now.addingTimeInterval(7_200)
        let existing = InterviewOpportunity(
            company: "Acme",
        ...
        let autoFollowUp = try XCTUnwrap(updated.followUps.first(where: { $0.linkedRoundID == updated.interviewRounds.first?.id }))
        XCTAssertEqual(updated.followUps.count, 2)
        XCTAssertTrue(updated.followUps.contains(where: {
            $0.action == "Old manual follow-up" && $0.dueDate == now.addingTimeInterval(-60)
        }))
        XCTAssertEqual(autoFollowUp.action, "Prepare for Technical")
        XCTAssertEqual(autoFollowUp.dueDate, futureRoundDate)
        XCTAssertTrue(autoFollowUp.wasAutoGenerated)
        XCTAssertEqual(updated.activeOrDueFollowUps(relativeTo: now).count, 2)
    }

    func testSavingCompletedRoundKeepsLinkedAutoFollowUpOpenUntilManuallyCompleted() throws {
        let now = Date(timeIntervalSince1970: 1_790_000_000)
        ...
        let existing = InterviewOpportunity(
            ...
            followUps: [
                InterviewFollowUp(
                    ...
                    dueDate: staleRound.date,
                    linkedRoundID: staleRound.id,
        ...
        let persistedFollowUp = try XCTUnwrap(updated.followUps.first)
        XCTAssertEqual(persistedFollowUp.action, "Prepare for Technical")
        XCTAssertEqual(persistedFollowUp.dueDate, staleRound.date)
        XCTAssertEqual(persistedFollowUp.linkedRoundID, staleRound.id)
        XCTAssertTrue(persistedFollowUp.wasAutoGenerated)
        XCTAssertTrue(updated.hasCurrentFollowUp(relativeTo: now))
        XCTAssertEqual(persistedFollowUp.status(relativeTo: now), .due)
    }

    func testSavingMultipleUpcomingRoundsCreatesMultipleAutoFollowUps() throws {
        let now = Date(timeIntervalSince1970: 1_790_000_000)
        let firstRoundDate = now.addingTimeInterval(3_600)
        let secondRoundDate = now.addingTimeInterval(7_200)
        let existing = InterviewOpportunity(
            company: "Acme",
            role: "iOS Engineer",
            status: .interviewing,
        ...
        XCTAssertEqual(updated.followUps.count, 2)
        XCTAssertEqual(
            Set(updated.followUps.compactMap { $0.linkedRoundID }),
            Set(updated.interviewRounds.map(\.id))
        )
        XCTAssertTrue(updated.followUps.allSatisfy { $0.wasAutoGenerated })
    }
    ...
    func testCalendarInterviewImportGroupsRoundsAndPreservesEventDetails() throws {
        ...
        let firstRoundAutoFollowUp = try XCTUnwrap(
            result.opportunities[0].followUps.first(where: {
                $0.linkedRoundID == result.opportunities[0].interviewRounds[0].id
            })
        )
        XCTAssertEqual(result.opportunities[0].followUps.count, 2)
        XCTAssertEqual(firstRoundAutoFollowUp.action, "Prepare for \(result.opportunities[0].interviewRounds[0].roundName)")
        XCTAssertEqual(firstRoundAutoFollowUp.dueDate, result.opportunities[0].interviewRounds[0].date)
        XCTAssertTrue(firstRoundAutoFollowUp.wasAutoGenerated)
    }
    ...
    func testCalendarInterviewReimportReconcilesScheduleStatusAndFollowUp() throws {
        let now = Date(timeIntervalSince1970: 1_790_000_000)
        ...
        XCTAssertTrue(pastImport.opportunities[0].hasCurrentFollowUp(relativeTo: now))
        XCTAssertEqual(
            pastImport.opportunities[0].activeOrDueFollowUps(relativeTo: now).first?.status(relativeTo: now),
            .due
        )
        ...
        let futureFollowUp = try XCTUnwrap(futureImport.opportunities[0].followUps.first)
        XCTAssertEqual(
            futureFollowUp.linkedRoundID,
            futureImport.opportunities[0].interviewRounds[0].id
        )
        XCTAssertTrue(futureFollowUp.wasAutoGenerated)
    }
    ...
    func testEligibleAgainFilteringResetsCompanyWhenNewInterviewEntryExists() {
        ...
        let olderEligibleOpportunity = InterviewOpportunity(
            ...
            followUps: [InterviewFollowUp(action: "Reapply later")],
    ...
    func testFilteredOpportunitiesSortByStartDateDescending() {
        ...
        let oldestOpportunity = InterviewOpportunity(
            ...
            followUps: [InterviewFollowUp(action: "Follow up soon", dueDate: oldestStart)],
